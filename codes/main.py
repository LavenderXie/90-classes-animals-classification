...
'''构建网络'''
device=torch.device('cuda'if torch.cuda.is_available() else "cpu")#电脑主机的选择,if torch.cuda.is_available() else "cpu"
net=vgg16(True, progress=True,num_classes=90)#定于分类的类别
net.to(device)
'''选择优化器和学习率的调整方法'''
lr=0.0001#定义学习率
optim=torch.optim.Adam(net.parameters(),lr=lr)#导入网络和学习率
sculer = torch.optim.lr_scheduler.CosineAnnealingLR(optim, T_max=20, eta_min=0)
#sculer=torch.optim.lr_scheduler.ExponentialLR(optim, gamma=0.95)#StepLR(optim,step_size=1)#步长为1的读取
'''训练'''
